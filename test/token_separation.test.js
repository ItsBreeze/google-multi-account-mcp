/**
 * MCP connector tokens and a host app's own user tokens must never validate
 * each other, despite both deriving from JWT_SECRET.
 *
 * This matters when the connector is mounted inside an existing application:
 * signing its tokens with the raw JWT_SECRET would make every connector token
 * a valid user token for that app.
 *   npm run test:tokens
 */
...
process.env.PUBLIC_BASE_URL = 'https://connector.example.com';

const o = require('../src/services/mcp_oauth.js');
const derived = crypto.createHmac('sha256', process.env.JWT_SECRET)
  .update('google-multi-account-mcp-v1').digest('base64');
...
check('MCP token is NOT accepted as a host-app user token', !asUser(mcpTok));
check('host-app user token is NOT accepted by the MCP guard', !accepts(() => o.verifyAccessToken(userTok)));
