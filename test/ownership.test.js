...
const THEIRS = {
  id: 'theirs', name: 'Quarterly Plan', mimeType: 'text/plain', ownedByMe: false, size: '10',
  owners: [{ emailAddress: 'owner@example.com' }],
};
const IN_DRIVE = {
  id: 'shared', name: 'Team Roster', mimeType: 'text/plain', ownedByMe: false, driveId: 'D9', owners: [],
};
...
(async () => {
  ...
  seen = record(THEIRS, (parsed) => (parsed.pathname.endsWith('/copy') ? { id: 'copy1', name: 'Quarterly Plan (copy)' } : {}));
  ...
  check('and names the owner it protected', String(res.body.note).includes('owner@example.com'), res.body.note);
  ...
  check('it names whose file it is', String(res.body.reason).includes('owner@example.com'), res.body.reason);
  check('it states the change precisely',
    res.body.changes[0].field === 'name' && res.body.changes[0].from === 'Quarterly Plan' && res.body.changes[0].to === 'Renamed',
  ...
  check('and the result does not hide whose file it was',
    res.body.edited_someone_elses_file === 'owner@example.com', JSON.stringify(res.body.edited_someone_elses_file));
