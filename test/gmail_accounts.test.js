...
process.env.DATABASE_URL = process.env.DATABASE_URL || 'postgresql://postgres@localhost:55432/connector_test';
...
(async () => {
  ...
  for (const email of ['personal@example.com', 'work@company.com', 'second@example.com']) {
  ...
  await expectThrow('ambiguous substring refuses to guess',
    () => resolveAccount('testowner','example.com'), 'be specific');
